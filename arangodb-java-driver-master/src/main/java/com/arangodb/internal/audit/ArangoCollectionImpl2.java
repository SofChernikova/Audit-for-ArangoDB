...
public class ArangoCollectionImpl2 extends InternalArangoCollection<ArangoDBImpl, ArangoDatabaseImpl, ArangoExecutorSync>
    ...
    public <T> MultiDocumentEntity<DocumentCreateEntity<T>> insertDocuments(Collection<T> values) throws ArangoDBException {
        MultiDocumentEntity<DocumentCreateEntity<T>> collections = insertDocuments(values,
                new DocumentCreateOptions().returnNew(true));
        try {
            Collection<DocumentCreateEntity<T>> documents =  collections.getDocuments();
            for (DocumentCreateEntity<T> entity: documents) {
                audit.insert(entity.getNew());
            }
        } catch (NullPointerException e) {
            LOGGER.info("WARN: you don't use audit for you document!");
        }
    ...
    @Override
    public <T> MultiDocumentEntity<DocumentCreateEntity<T>> insertDocuments(final Collection<T> values, final DocumentCreateOptions options)
    ...
    public <T> DocumentUpdateEntity<T> replaceDocument(String key, T value) throws ArangoDBException {
        DocumentUpdateEntity<T> documentUpdate = replaceDocument(key, value,
                new DocumentReplaceOptions().returnNew(true).returnOld(true));
        try {
            audit.replace(documentUpdate.getNew());
//            audit.send(documentUpdate.getOld());
        } catch (NullPointerException e) {
            LOGGER.info("WARN: you don't use audit for you document!");
        }
    ...
    @Override
    public <T> DocumentUpdateEntity<T> replaceDocument(final String key, final T value, final DocumentReplaceOptions options)
            throws ArangoDBException {
    ...
    public <T> MultiDocumentEntity<DocumentUpdateEntity<T>> replaceDocuments(Collection<T> values) throws ArangoDBException {
        ...
        try {
            Collection<DocumentUpdateEntity<T>> documents =  collections.getDocuments();
            for (DocumentUpdateEntity<T> entity: documents) {
                audit.replace(entity);
            }
        } catch (NullPointerException e) {
            LOGGER.info("WARN: you don't use audit for you document!");
        }
    ...
    public <T> DocumentUpdateEntity<T> updateDocument(String key, T value) throws ArangoDBException {
        DocumentUpdateEntity<T> documentUpdate = updateDocument(key, value,
                new DocumentUpdateOptions().returnNew(true).returnOld(true));
        try {
            audit.update(documentUpdate.getNew());
        } catch (NullPointerException e) {
            LOGGER.info("WARN: you don't use audit for you document!");
        }
        return documentUpdate;
    ...
    public <T> MultiDocumentEntity<DocumentUpdateEntity<T>> updateDocuments(Collection<T> values) throws ArangoDBException {
        MultiDocumentEntity<DocumentUpdateEntity<T>> collections = updateDocuments(values,
                new DocumentUpdateOptions().returnNew(true).returnOld(true));
        try {
            Collection<DocumentUpdateEntity<T>> documents =  collections.getDocuments();
            for (DocumentUpdateEntity<T> entity: documents) {
                audit.update(entity.getNew());
            }
        } catch (NullPointerException e) {
            LOGGER.info("WARN: you don't use audit for you document!");
        }
